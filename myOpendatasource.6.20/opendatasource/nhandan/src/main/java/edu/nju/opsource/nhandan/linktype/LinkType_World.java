...
public class LinkType_World extends LinkType{
    
	 private String pageContent = null;
     private Document doc = null;
	 private Data_World data = null;
	 ...
	 public LinkType_World(String linkTypeName) {
		 ...
		 /*  36 */     super.setLinkTypeName("nhandan.world");
		 /*     */   }
	...
	private void getDataFromDoc() {
		...
		Data_World data_World = new Data_World();
		     ...
		     data_World.setContent(content.toString());
		     data_World.setContentWithTag(contentWithTag.toString());
		 
		     data_World.setDate(getLinkDate() == null ? new Date() : getLinkDate());
		 /*
		  * select
		  */
		     Elements titleE = this.doc.select("div.fontM.ndtitle h3");
		     if ((titleE == null) || (titleE.size() == 0)) data_World.setTitle("NULLDATAFIELD"); else {
		    	 data_World.setTitle(titleE.first().text());
		     }
		     Elements abstractE = this.doc.select("div.ndcontent.ndb p");
		     if ((abstractE == null) || (abstractE.size() == 0)) data_World.setAbstract("NULLDATAFIELD"); else {
		    	 data_World.setAbstract(abstractE.first().text());
		     }
		     /*
		      * src
		      */
		     Elements titleImgE = this.doc.select("img.nd_img");
		     if ((titleImgE == null) || (titleImgE.size() == 0)) data_World.setImgUri("NULLDATAFIELD"); else {
		    	 data_World.setImgUri("http://en.nhandan.com.vn/"+titleImgE.first().attr("src"));
		     }
		     this.data = data_World;
	}
