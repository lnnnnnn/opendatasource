...
public class LinkType_WorldList extends LinkType{
	...
	public boolean handle(Link parentlink) throws Exception {
     ...
     for (Element newItem : news) {
       ...
       {
         Link link = new Link(tmp.first().attr("abs:href"), new LinkType_World(), ELinkState.UNCRAWL, parentlink, this.site);
     ...
     if ((nextPageE != null) && (nextPageE.size() != 0)) {
       ...
       this.nextPage = new Link(nextPageE
         .first().attr("abs:href"), new LinkType_WorldList(this.site), ELinkState.UNCRAWL, parentlink, this.site);
	...
	public LinkedList<Link> getNewItems() {
		...
		public String toString() {
			return "LinkType_WorldList [doc=" + doc + ", site=" + site + ", newItems=" + newItems + ", nextPage="
					+ nextPage + "]";
		}
