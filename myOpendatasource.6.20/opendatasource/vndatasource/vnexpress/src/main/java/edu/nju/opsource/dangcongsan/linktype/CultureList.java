...
public class CultureList extends LinkType {
	...
	public CultureList(String linkTypeName) {
		super.setLinkTypeName(linkTypeName);
	}

	public CultureList(Site site) {

		super.setLinkTypeName("dangsangsan.culturelist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			Element tmp = newItem;
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new Culture(), ELinkState.UNCRAWL, parentlink,
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new CultureList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
