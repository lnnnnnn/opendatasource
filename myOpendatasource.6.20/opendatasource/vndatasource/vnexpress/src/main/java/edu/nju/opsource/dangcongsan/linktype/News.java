...
import edu.nju.opsource.vnexpress.VnexpressOpenDataSource;
		...
		for (Element image : images) {
			...
			if (graphs == null || graphs.size() == 0) {
				...
				int rst = GetImage.downloadImage(url,
						VnexpressOpenDataSource.imagePath + url.substring(url.lastIndexOf("/") + 1), 4096);
				if (rst == GetImage.Success) {
					image.attr("src", url.substring(url.lastIndexOf("/") + 1));
					System.out.println(".... crawled");
				}
				new Data_Graph().setUrlMD5(urlMD5).setUrl(url)
						.setFile(VnexpressOpenDataSource.imagePath + url.substring(url.lastIndexOf("/") + 1))
						.insert();
			}
		}
	}
