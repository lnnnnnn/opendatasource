...
public class ScienceList extends LinkType {
	...
	public ScienceList(String linkTypeName) {
		super.setLinkTypeName(linkTypeName);
	}

	public ScienceList(Site site) {

		super.setLinkTypeName("dangsangsan.sciencelist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			Element tmp = newItem;
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new Science(), ELinkState.UNCRAWL, parentlink,
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new ScienceList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
