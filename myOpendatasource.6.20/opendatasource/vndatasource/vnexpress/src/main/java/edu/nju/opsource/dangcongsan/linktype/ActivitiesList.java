...
public class ActivitiesList extends LinkType {
	...
	public ActivitiesList(String linkTypeName) {
		super.setLinkTypeName(linkTypeName);
	}

	public ActivitiesList(Site site) {

		super.setLinkTypeName("dangsangsan.activitieslist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			Element tmp = newItem;
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new Activities(), ELinkState.UNCRAWL, parentlink,
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new ActivitiesList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
