...
public class OverseasList extends LinkType {
	...
	public OverseasList(String linkTypeName) {
		super.setLinkTypeName(linkTypeName);
	}

	public OverseasList(Site site) {

		super.setLinkTypeName("dangsangsan.overseaslist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			Element tmp = newItem;
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new Overseas(), ELinkState.UNCRAWL, parentlink,
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new OverseasList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
