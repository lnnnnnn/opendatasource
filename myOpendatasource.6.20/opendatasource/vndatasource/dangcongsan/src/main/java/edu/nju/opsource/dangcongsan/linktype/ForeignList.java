...
public class ForeignList extends LinkType {
	...
	public ForeignList(String linkTypeName) {
		super.setLinkTypeName(linkTypeName);
	}

	public ForeignList(Site site) {

		super.setLinkTypeName("dangsangsan.foreignlist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			Element tmp = newItem;
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new Foreign(), ELinkState.UNCRAWL, parentlink,
		...
		//	this.doc.select("li#dnn_ctr5043_newsviewer_ctl00_vbPaging__Previous").first().nextElementSibling());
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new ForeignList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
