...
public class NewsList extends LinkType {
	...
	public NewsList(String linkTypeName) {
		super.setLinkTypeName(linkTypeName);
	}

	public NewsList(Site site) {

		super.setLinkTypeName("dangsangsan.newslist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			Element tmp = newItem;
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new News(), ELinkState.UNCRAWL, parentlink,
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new NewsList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
	...
	public static void main(String[] args) {
		NewsList lp = new NewsList();
		lp.get("http://en.dangcongsan.vn/news-and-events.html");
		try {
			lp.handle(new Link());
		} catch (Exception e) {
