...
public class EconomicsList extends LinkType {
	...
	public EconomicsList() {
		super();
		// TODO Auto-generated constructor stub
	}

	public EconomicsList(String linkTypeName) {
		super.setLinkTypeName(linkTypeName);
	}

	public EconomicsList(Site site) {

		super.setLinkTypeName("dangsangsan.economicslist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			Element tmp = newItem;
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new Economics(), ELinkState.UNCRAWL, parentlink,
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new EconomicsList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
