...
public class LinkType_PoliticsList extends LinkType {
	...
	public LinkType_PoliticsList(Site site) {
		...
		super.setLinkTypeName("vietnameplus.politicslist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			...
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new LinkType_Politics(), ELinkState.UNCRAWL, parentlink,
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new LinkType_PoliticsList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
	...
	public static void main(String[] args) {
		LinkType_PoliticsList lp = new LinkType_PoliticsList();
		lp.get("http://en.vietnamplus.vn/politics/page893.vnp");
		try {
			lp.handle(new Link());
		} catch (Exception e) {
