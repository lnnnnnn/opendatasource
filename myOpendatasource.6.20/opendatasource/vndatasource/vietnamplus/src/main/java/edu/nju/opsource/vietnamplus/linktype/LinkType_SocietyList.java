...
public class LinkType_SocietyList extends LinkType {
	...
	public LinkType_SocietyList(Site site) {
		...
		super.setLinkTypeName("vietnameplus.societylist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			...
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new LinkType_Society(), ELinkState.UNCRAWL, parentlink,
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new LinkType_SocietyList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
