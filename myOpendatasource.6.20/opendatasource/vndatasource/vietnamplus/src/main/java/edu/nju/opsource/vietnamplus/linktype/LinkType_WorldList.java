...
public class LinkType_WorldList extends LinkType {
	...
	public LinkType_WorldList(Site site) {
		...
		super.setLinkTypeName("vietnameplus.worldlist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			...
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new LinkType_World(), ELinkState.UNCRAWL, parentlink,
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new LinkType_WorldList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
