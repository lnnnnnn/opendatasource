...
public class LinkType_BusinessList extends LinkType {
	...
	public LinkType_BusinessList() {
		super();
		// TODO Auto-generated constructor stub
	}

	public LinkType_BusinessList(String linkTypeName) {
		super.setLinkTypeName(linkTypeName);
	}

	public LinkType_BusinessList(Site site) {

		super.setLinkTypeName("vietnameplus.businesslist");
		this.site = site;
	}
	...
	public boolean handle(Link parentlink) throws Exception {
		...
		for (Element newItem : news) {
			...
			if ((tmp != null)) {
				Link link = new Link(tmp.attr("abs:href"), new LinkType_Business(), ELinkState.UNCRAWL, parentlink,
		...
		if (nextPageE != null) {
			...
			this.nextPage = new Link(url, new LinkType_BusinessList(this.site), ELinkState.UNCRAWL, parentlink,
					this.site).setLinkDate(new java.sql.Date(new Date().getTime()));
